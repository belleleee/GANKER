'use strict';
            ...
            function mwDir(yMin) {
                for (let k = 0; k < 40; k++) {
                    ...
                    const g = (Math.random() + Math.random() + Math.random() - 1.5) / 1.5 * 0.30;
            ...
            for (let i = 0; i < STAR_COUNT; i++) {
                ...
                starSpeed[i] = 0.45 + Math.random() * 1.55;
                starColorMix[i] = (Math.random() + Math.random()) * 0.5;
                starSize[i] = Math.random() < 0.07 ? 6.4 + Math.random() * 2.8 : 2.9 + Math.random() * 2.3;
            }
            {
                let wi = STAR_COUNT;
                while (wi < TOTAL_STARS) {
                    ...
                    starTwinkle[wi] = Math.random() * Math.PI * 2;
                    starSpeed[wi] = 0.2 + Math.random() * 0.85;
                    starColorMix[wi] = Math.random() < 0.82 ? Math.random() * 0.32 : Math.random();
                    starSize[wi] = Math.random() < 0.05 ? 2.3 + Math.random() * 1.3 : 0.9 + Math.random() * 1.3;
                    wi++;
                }
            }
        ...
        void main() {
          ...
          gl_FragColor = vec4(starColor, min(alpha, 1.0));
            ...
            function spawnMeteor(m) {
                const th = Math.random() * Math.PI * 2;
                const ph = Math.acos(0.25 + Math.random() * 0.65);
                const r = 78;
                ...
                m.vy = -(18 + Math.random() * 26);
                m.life = 0.75 + Math.random() * 0.5;
                m.t = 0; m.active = true;
            }
            function updateMeteors(dt, skyVis) {
                ...
                if (meteorTimer <= 0) {
                    ...
                    meteorTimer = 3.5 + Math.random() * 7.5;
                }
            ...
            function makeCloud() {
                const g = new THREE.Group(); const n = 4 + Math.floor(Math.random() * 4); let cx = 0;
                for (let i = 0; i < n; i++) {
                    const w = 1.8 + Math.random() * 2.0, h = 1.2 + Math.random() * 1.0, d = 1.8 + Math.random() * 2.0;
                    const px = cx, py = (i > 0 && Math.random() < 0.4) ? h * 0.3 : (Math.random() - 0.3) * 0.2, pz = (Math.random() - 0.5) * 1.2;
                ...
                g.position.set((Math.random() - 0.5) * 90, 18 + Math.random() * 8, -30 + Math.random() * 60); g.visible = false;
                const c = { grp: g, op: 0, spd: 0.7 + Math.random() * 0.6, floatPh: Math.random() * 6.28 }; cloudList.push(c); scene.add(g); return c;
            }
            ...
            const rainDrops = [];
            for (let i = 0; i < RAIN_MAX; i++) rainDrops.push({ x: (Math.random() - 0.5) * 34, y: Math.random() * 14, z: (Math.random() - 0.5) * 34, v: 9 + Math.random() * 4 });
            ...
            for (let i = 0; i < SNOW_MAX; i++) {
                ...
                snowFlakes.push({ x: (Math.random() - 0.5) * 34, y: Math.random() * 14, z: (Math.random() - 0.5) * 34, v: 0.6 + Math.random() * 0.5, ph: Math.random() * 6.28, amp: 0.35 + Math.random() * 0.4, fq: 0.5 + Math.random() * 0.7, rot: Math.random() * 6.28, rotV: (Math.random() - 0.5) * 2, ux, uy, uz, vx, vy, vz });
            }
            ...
            for (let i = 0; i < SNOW_BALL_MAX; i++) snowBallFlakes.push({ x: (Math.random() - 0.5) * 34, y: Math.random() * 14, z: (Math.random() - 0.5) * 34, v: 0.5 + Math.random() * 0.4, ph: Math.random() * 6.28, amp: 0.2 + Math.random() * 0.3, fq: 0.6 + Math.random() * 0.5 });
            ...
            let boltT = 0, nextBolt = 6 + Math.random() * 12;
            function spawnBolt() { const pts = []; let bx = -10 + Math.random() * 20, bz = -14 + Math.random() * 10, by = 13; pts.push(V(bx, by, bz)); while (by > 0.5) { by -= 1.5 + Math.random() * 1.5; bx += (Math.random() - 0.5) * 2.6; bz += (Math.random() - 0.5) * 1.2; pts.push(V(bx, Math.max(by, 0), bz)); } boltLine.geometry.dispose(); boltLine.geometry = geo(pts.map(p => [p.x, p.y, p.z])); boltLine.visible = true; }
            ...
            function updateWeatherSystem(dt, time) {
                ...
                for (let i = 0; i < cloudList.length; i++) {
                    ...
                    if (c.grp.visible) {
                        ...
                        if (c.grp.position.x > 60) { c.grp.position.x = -60; c.grp.position.z = -30 + Math.random() * 60; }
                ...
                if (rainLines.visible) {
                    ...
                    for (let i = 0; i < rainN; i++) {
                        ...
                        if (d.y < roofTopAt(d.x, d.z) || tY < roofTopAt(tX, tZ) || d.y < 0) {
                            d.y = 11 + Math.random() * 3; d.x = (Math.random() - 0.5) * 34; d.z = (Math.random() - 0.5) * 34;
                        }
                ...
                if (snowLines.visible) {
                    ...
                    for (let i = 0; i < snowN; i++) {
                        ...
                        if (f.y < roofTopAt(f.x, f.z) + arm || f.y < 0) { f.y = 11 + Math.random() * 3; f.x = (Math.random() - 0.5) * 34; f.z = (Math.random() - 0.5) * 34; }
                ...
                if (snowBalls.visible) {
                    ...
                    for (let i = 0; i < snowBN; i++) {
                        ...
                        if (f.y < roofTopAt(f.x, f.z) + 0.08 || f.y < 0) { f.y = 11 + Math.random() * 3; f.x = (Math.random() - 0.5) * 34; f.z = (Math.random() - 0.5) * 34; }
                        snowBallPos[idx++] = f.x; snowBallPos[idx++] = f.y; snowBallPos[idx++] = f.z;
                    }
                    snowBallGeo.attributes.position.needsUpdate = true;
                }

                if (wx.type === 'storm' && wx.rain > 260) { nextBolt -= dt; if (nextBolt <= 0) { spawnBolt(); boltT = 0.45; nextBolt = 6 + Math.random() * 14; } }
