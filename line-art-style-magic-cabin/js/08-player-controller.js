'use strict';
            ...
            function aimRay() { raycaster.setFromCamera(CENTER, camera); const h = raycaster.intersectObjects(hingeMeshes, false).filter(x => ancestorVisible(x.object)); if (h.length) { const g = h[0].object.userData.hingeGroup; return { label: g.userData.aimLabel || '交互', act: () => toggleSpring(g) }; } const m = raycaster.intersectObjects(magicMeshes, false).filter(x => ancestorVisible(x.object)); if (m.length) { const o = m[0].object.userData.magicRoot; return { label: o.userData.aimLabel || '交互', magic: true, act: () => fireMagic(o) }; } const f = raycaster.intersectObjects(fireMeshes, false).filter(x => ancestorVisible(x.object)); if (f.length) return { label: '点燃 / 熄灭壁炉', act: toggleFire }; return null; }
            const isLocked = () => document.pointerLockElement === renderer.domElement;
            function doInteract() { const target = viewMode === 'fp' && (aimHit || IS_TOUCH) ? aimHit : nearestInteract; if (!target) return; if (typeof window.noteAchievementEvent === 'function') window.noteAchievementEvent('interaction', { label: target.label || '交互', magic: !!target.magic }); target.act(); }
