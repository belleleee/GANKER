'use strict';
            ...
            function showHintOverride(html) { hintOverrideText = html; hintOverrideUntil = performance.now() * 0.001 + 2.4; }
            ...
            const gatherCol = new Float32Array(GATHER_N * 3);
            {
                for (let i = 0; i < GATHER_N; i++) {
                    ...
                    gatherData.push({
                        ...
                        r0: 8 + Math.random() * 12, spd: 0.3 + Math.random() * 0.55, ph: Math.random() * Math.PI * 2
                    });
          ...
          void main() {
            ...
            function fillStarAttrs(g, n) {
                ...
                for (let i = 0; i < n; i++) {
                    ...
                    ph[i] = Math.random(); an[i] = Math.random() * Math.PI; sz[i] = 2.4 + Math.random() * 3.2;
                }
            ...
            fillStarAttrs(castStarGeo, CAST_STAR_N);
            {
                ...
                for (let i = 0; i < CAST_STAR_N; i++) {
                    const th = Math.random() * Math.PI * 2, rr = 6 + Math.random() * 8.5;
                    pos[i * 3] = Math.cos(th) * rr;
                    pos[i * 3 + 1] = -6 + Math.random() * 13;
            ...
            const dustGeo = new THREE.BufferGeometry();
            {
                ...
                for (let i = 0; i < DUST_N; i++) {
                    const th = Math.random() * Math.PI * 2, rr = Math.random() * 7;
                    pos[i * 3] = Math.cos(th) * rr; pos[i * 3 + 1] = 0; pos[i * 3 + 2] = Math.sin(th) * rr;
                    const c = [0.55, 0.85, 1];
                    col[i * 3] = c[0]; col[i * 3 + 1] = c[1]; col[i * 3 + 2] = c[2];
                    seed[i] = Math.random(); spd[i] = 0.9 + Math.random() * 1.3;
                }
          ...
          gl_FragColor = vec4(vC * 1.2, a);
            ...
            function spawnExplosion(c) {
                ...
                for (let i = 0; i < 22; i++) {
                    ...
                    const sp = 3 + Math.random() * 8.5;
                    const vel = V(Math.cos(a) * sp, high ? 5.5 + Math.random() * 8 : 1 + Math.random() * 4, Math.sin(a) * sp);
                    const r0 = 0.8 + Math.random() * 0.8;
                    const life = 1.7 + Math.random() * 0.9;
                ...
                // 10) 飞散碎屑
                {
                    ...
                    for (let i = 0; i < 26; i++) {
                        ...
                        const sc = 0.1 + Math.random() * 0.24;
                        const a = Math.random() * Math.PI * 2, up = 4 + Math.random() * 10;
                        const sp = 4 + Math.random() * 10;
                        const vel = V(Math.cos(a) * sp, up, Math.sin(a) * sp);
                        const rotV = V((Math.random() - 0.5) * 12, (Math.random() - 0.5) * 12, (Math.random() - 0.5) * 12);
                ...
                // 11) 火花
                {
                    ...
                    for (let i = 0; i < N; i++) {
                        ...
                        const th = Math.random() * Math.PI * 2, ph = Math.acos(Math.random() * 1.6 - 0.6);
                ...
                // 12) 彩色余烬
                {
                    ...
                    for (let i = 0; i < N; i++) {
                        pos[i * 3] = c.x; pos[i * 3 + 1] = c.y; pos[i * 3 + 2] = c.z;
                        const th = Math.random() * Math.PI * 2, ph = Math.acos(Math.random() * 1.4 - 0.7);
                ...
                // 16) 地面焦痕 + 放射状龟裂（残效）
                {
                    ...
                    for (let i = 0; i < 16; i++) {
                        let ang = i / 16 * Math.PI * 2 + Math.random() * 0.25;
                        let r = 1.4;
                        const len = 5 + Math.random() * 5.5, steps = 7;
                        const pts = [];
                        for (let k = 0; k <= steps; k++) {
                            pts.push(V(c.x + Math.cos(ang) * r, 0.03, c.z + Math.sin(ang) * r));
                            r += len / steps;
                            ang += (Math.random() - 0.5) * 0.24;
                        }
